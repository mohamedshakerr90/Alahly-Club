...
console.log("6a");
  ...
  //  console.log(i);
    datalow[i.toLowerCase()]=result.data[i];
   // delete obj[i];
}
...
//alert(self.companyid);
    ...
    var Scanning = AbstractAction.extend({
        template: "Scanning",
 xmlDependencies: ['/stock_scan_frontend/static/src/xml/stock.xml'],
...
console.log("6");
                    ...
                    if (isNaN(dateCheck.valueOf())) {
                        ...
                        if (self.stock_data.data[scan_box]) {
                            ...
                            if (scan == false) {
                                ...
                                self.stock_data.createdScansObj[scan_box][codeProduct['id']] = {'product_id': codeProduct['id'],'company_id':this.stock_data.companyid, 'name': scan_box,  'ref': default_code};
       ...
       // alert(this.stock_data.companyid);
        console.log("13");
            ...
            var scanlower=scan_box.toLowerCase();
            if (self.stock_data.datalower[scanlower] != undefined) {
           //// var countProducts = Object.keys(self.stock_data.data[scan_box]).length;
                      //      if (countProducts == 1) {
                            var scan = self.stock_data.datalower[scanlower][0];
                            scan_box=scan.lot_no;
                       //     }
            }
...
//                                    lot_name = scan_box.split('/')[1].slice(5, -4);
                                    ...
                                    if (self.stock_data.type_of_scaning == 'first_group') {
                                        ...
                                        self.stock_data.createdScansObj[scan_box][codeProduct['id']] = {'product_id': codeProduct['id'],'company_id': this.stock_data.companyid , 'name': scan_box, 'ref': codeProduct['default_code']};
